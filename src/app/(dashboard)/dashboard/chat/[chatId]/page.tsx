...
export async function generateMetadata({
  ...
  return { title: `dot. | ${chatPartner.name}` }
}
...
const page: FC<PageProps> = async ({ params }: PageProps) => {
  ...
  const chatParterId = user.id === userId1 ? userId2 : userId1
  const chatPartner = (await db.get(`user:${chatParterId}`)) as User
  const initialMessages = await getChatMessages(chatId)

  return (
    <div className="flex-1 justify-between flex flex-col h-full max-h-[calc(100vh-6rem)]">
      <div className="box-shadow-lg flex sm:items-center justify-between py-3 border-gray-200">
